...
class ConDB:
    def __init__(self, connstr=None, connection=None):
        #print("ConDB: created with connection:", type(connection), connection)
    ...
    def connect(self):
        if self.Conn == None:
            ...
            #print("ConDB.connect(): reconnected:", self.Conn)
        #print("ConDB.connect(): returning:", self.Conn)
        return self.Conn
    
    def cursor(self):
        conn = self.connect()
        c = conn.cursor()
        #print("ConDB.cursor(): conn:", conn, "   cursor:", c)
        return c
...
"""
    ...
    def getData__(self, t, tag=None, tr=None, data_type=None, channel_range=None, conditions=[]):
        # returns {channel:(data,)},{channel:tv}
        #print "getData: tr=%s" % (tr,)
        s = self.findSnapshot(t, tag=tag, tr=tr, data_type=data_type)
        if s == None: return {}, {}
        #print 'snapshot for t=%s: %s' % (t,s)
        data0 = s.getData(channel_range = channel_range, conditions = conditions)
        #print "table.getData: creating dictionary..."
        data = {}
        outv = dict_with_default(s.Tv)
        for tup in data0:
            channel = tup[0]
            data[channel] = tup[1:]
        #print "table.getData: calling getUpdates()..."
        updates = s.getUpdates(t, tr, channel_range = channel_range, conditions = conditions)
        #print "table.getData: got updates"
        for tup in updates:
            channel = tup[0]
            utv = tup[1]
            outv[channel] = utv
        ...
        if data_type != None:
            ...
            if common_data:
                for channel, common_tup in common_data.items():
                    ...
                    if channel not in data or ctv > outv[channel]:
                        data[channel] = common_tup
                        outv[channel] = ctv
        #print "return from table.getData"
        return data, outv
    ...
    def getDataIntervalIter(self, t1, t2, tag=None, tr=None, data_type=None, channel_range=None,
        ...
        if data_type != None:
            ...
            snapshots.sort(key=lambda x: x.Tv)
    ...
    def createSnapshot(self, t, prefill, data_type=None):
        s = CDSnapshot.create(self, t, data_type)
        s.addData(prefill)
        #print "Added data:", len(data)
        return s

    def createSnapshot_(self, t, prefill, data_type=None):
        base = []
        if prefill:
            base = self.getDataIter(t, data_type=data_type)
            base = list(base)
            #print "createSnapshot: base=%s" % (base,)
        base_iter = ((tup[0], tup[2:]) for tup in base)
        s = CDSnapshot.create(self, t, data_type)
        s.addData(prefill)
        #print "Added data:", len(data)
        return s
