...
class CDSnapshot:
    ...
    def getAllUpdates(self):
        ...
        if self.Updates == None:
            ...
            for tup in cursor_iterator(c):
                channel, tv, tr = tup[:3]
                if channel > last_channel or tv > last_tv or tr > last_tr:
    ...
    def getUpdatesForTimeFromCache(self, tv, tr = None, channel_range = None, conditions = []):
        # returns iterator [(channel,tv,data,...),...]
        for channel, lst in self.Updates.iteritems():
    ...
    def getValuesIter(self, t, tr = None, channel_range = None, conditions = []):
        ...
        for tup in updates:
            ...
            if channel_range is None or (
                    channel_range[0] <= channel and channel <= channel_range[1]):
                tv = tup[1]
                if data.has_key(channel):   del data[channel]
                #print "s.getValuesIter: yielding ", tup
                yield tup
        for channel, (tv, values) in data.iteritems():
            if channel_range is None or (
                    channel_range[0] <= channel and channel <= channel_range[1]):
    ...
    def addData__(self, data):
        ...
        dmp = []
        for channel, values in data.iteritems():
...
class Segment:
    ...
    def shadow(self, snapshot):
        ...
        return [Segment(self.Snapshot, t0, t1) for t0, t1, flag in segments if flag == 1 and (t1 > t0 or t1 == None)]
...
"""
    ...
    def overlaySnapshots(self, lst):
        # assumes lst ir orderd by Tr, so subsequent snapshots override previous ones, if there is an overlap
        ...
        out.sort(lambda x, y:   cmp(x.Tv, y.Tv))    # sort by Tv
        return out
    ...
    def findSegments(self, t1, t2, tag=None, tr=None, data_type=None):
        ...
        return self.overlaySnapshots(lst)
    ...
    def getDataIntervalIter(self, t1, t2, tag=None, tr=None, data_type=None, channel_range=None,
        ...
        if data_type != None:
            ...
            segments.sort(lambda x,y: cmp(x.Tv, y.Tv))
        ...
        for sg in segments:
            ...
            for channel, ulst in updates.iteritems():
                for tv, data in ulst:
                    if tv >= ta and tv < tb:
    ...
    def getDataInterval(self, t1, t2, tag=None, tr=None, data_type=None, channel_range=None,
        ...
        #
        lst = list(self.getDataIntervalIter(t1, t2, tag=tag, tr=tr, data_type=data_type, 
                            channel_range=channel_range, conditions=conditions))
        lst.sort()
        return lst
    ...
    def patch(self, data, tbegin, tend, data_type=None):
        channels = {}
        for channel, tv, data in data:
            channels[channel] = 1
        end_data = [(channel, tend, data) for channel, tv, data in self.getDataIter(tend, data_type = data_type)
                            if channels.has_key(channel)]   # record only end values for channels which were
                                                            # actually patched
        s = self.createSnapshot(tbegin, False, data_type=data_type, tv_end = tend)
        s.addUpdates(data)
        s.addUpdates(end_data)
    ...
    def addData(self, data, tolerances = None, data_type=None):
        ...
        if new_snapshot:
            ...
            for tup in self.getDataIter(tmin, data_type=data_type):
                ...
                if not data_time_dict.has_key(channel) or data_time_dict[channel] != tmin:
