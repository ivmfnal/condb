...
class CDSnapshot:
    ...
    @staticmethod
    def create(table, tv, data_type, initial_data):
        # initial data: {channel: (values,...)}
        ...
        s.fetchInfo()
        if initial_data:    s.addData(initial_data)
        return s
    ...
    def addData(self, data):
        ...
        #k = data.keys()[0]
        #print data[k]
        format = ["%s" % (self.Id,), "%s" % (self.Tv,), "%d"] + ["%s"]*len(data_columns)
        format = '\t'.join(format)
        ...
        c = self.Table.copy_from(dmp, "%t_update", 
            ['__snapshot_id', '__tv', '__channel']+data_columns)
        c.execute("commit")
        self.invaludateCache()
        self.T['addData'].end()
    ...
    def getData(self, channel_range = channel_range, conditions = conditions):
        self.fetchInfo()
        return {}, self.Tv
    ...
    def getUpdates(self, tv, tr, channel_range = None, conditions = []):
...
(
    ...
    def getData(self, t, tag=None, tr=None, data_type=None, channel_range=None,
        ...
        d0, tv = s.getData(channel_range = channel_range, conditions = conditions)
        data = d0.copy()
        #print data
        updates, validity = s.getUpdates(t, tr, channel_range = channel_range, conditions = conditions)
    ...
    def getDataInterval__(self, t1, t2, tag=None, tr=None, data_type=None, channel_range=None,
                conditions=[]):
        ...
        begin_data, begin_validity = self.getData(t1, tag=tag, tr=tr, data_type=data_type, channel_range=channel_range)
        for channel, data in begin_data.items():
            tv = begin_validity[channel]
            output[channel] = [(tv, data)]

        for s in snapshots:
            #print "calling getData..."
            #print "calling getUpdates..."
            updates = s.getUpdatesInRange(t1, t2, tr, channel_range = channel_range, conditions = conditions)
    ...
    def getDataInterval(self, t1, t2, tag=None, tr=None, data_type=None, channel_range=None):
        ...
        #
        output = {}
        for channel, tv, data in self.getDataIntervalIter(t1, t2, tag=tag, tr=tr, data_type=data_type, 
                            channel_range=channel_range):
            lst = output.get(channel)
            if not lst:
                lst = []
                output[channel] = lst
            lst.append((tv, data))
    ...
    def createSnapshot(self, t, prefill = True, data = {}, data_type=None):
        ...
        if prefill:
            ...
            base.update(data)
            data = base
        s = CDSnapshot.create(self, t, data_type, data)
        return s
        ...
        if new_snapshot:
            ...
            for chn, tv, values in data:
                if tmin == tv:
                    ...
                    updates.append((chn, tv, values))
            #print 'data0=', data0
            s0 = CDSnapshot.create(self, tmin, data_type, data0)
            data = updates
